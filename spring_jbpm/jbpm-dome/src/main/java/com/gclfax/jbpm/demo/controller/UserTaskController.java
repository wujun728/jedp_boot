...
public class UserTaskController {
    ...
    @ResponseBody
    public List<Long> getTasksAll(@RequestParam String processInstanceId) {

        List<Long> tasks = runtimeDataService.getTasksByProcessInstanceId(Long.parseLong(processInstanceId));
    ...
    @ResponseBody
    public List<AuditTask> groupTaskList() {
        ...
        String groupStr = "";
        if (role != null && role.size() > 0 && !StringUtil.isBlank(role.get(0).getName())) {
    ...
    @ResponseBody
    public String completeTask(@RequestParam String id, @RequestParam Integer tid,@RequestParam Integer pid,@RequestParam Integer day) {
        String userId = getAuthUser();
...
//        }
    ...
    @ResponseBody
    public String claimTask(@RequestParam String id, @RequestParam Integer tid,@RequestParam Integer pid) {
        String userId = getAuthUser();
        Map<String, Object> map = new HashMap<>();
        try {
    ...
    @ResponseBody
    public String startTask(@RequestParam String id, @RequestParam Integer tid,@RequestParam Integer pid) {
        String userId = getAuthUser();
        Map<String, Object> map = new HashMap<>();
        try {
    ...
    public String activateTask(@RequestParam String id) {
        ...
        try {
            ...
            map.put("msg", "Task " + id + " activate successfully");
            return JSONObject.fromObject(map).toString();

        } catch (Exception e) {
            map.put("msg", "Task " + id + " activate failed due to " + e.getMessage());
            return JSONObject.fromObject(map).toString();
        }
