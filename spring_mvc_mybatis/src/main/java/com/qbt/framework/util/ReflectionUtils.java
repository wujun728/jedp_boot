...
/**<p>Class: ReflectionUtils.java</p>
...
public class ReflectionUtils {
	...
	protected static Logger logger = LogManager.getLogger(ReflectionUtils.class);
	...
	public static Object getFieldValue(final Class clazz, final String fieldName) {
		...
		Object obj = null;
		if( !Modifier.isStatic(field.getModifiers())){				//非静态字段
			try {
				obj = clazz.newInstance();
			} catch (Exception e) {
				logger.error( e.getMessage());
				throw new RuntimeException(clazz + " must be have empty constructor and can access");
			}
		}
		try {
	 ...
	 * 用于一次性调用的情况.
	 */
	public static Object invokeMethod(final Class<?> clazz, final String methodName, final Object[] args, final Class<?>... parameterTypes) {
		...
		Object obj = null;
		if( !Modifier.isStatic(method.getModifiers())){				//非静态字段
			try {
				obj = clazz.newInstance();
			} catch (Exception e) {
				logger.error( e.getMessage());
	...
	public static Object invokeMeth(final Object[] args, final Class<?> clazz, final String methodName ) {
		return invokeMethod(clazz, methodName, args, null);
	}
