...
layui.define('fly', function(exports){
  ...
  gather.jiedaActive = {
    ...
    ,reply: function(li){ //回复
      ...
      var author =  li.find('.fly-detail-user cite').text().replace(/\s/g, '');
      dom.content.focus();
      if(val.indexOf(aite) !== -1) return;
      dom.content.val(aite +' ' );
      // dom.author.attr("value",author);
      dom.author.val(author);
    }
    ,accept: function(li){ //采纳
      var othis = $(this);
      layer.confirm('是否采纳该回答为最佳答案？', function(index){
        layer.close(index);
        fly.json('/Forum/AcceptReply', {
          id: li.data('id') //data-id传过来
        }, function(res){
          if(res.status === 0){
            $('.jieda-accept').remove();
