...
function del(){
	...
	if(row.length!=0) {
		window.location='${pageContext.request.contextPath}/activiti/activiti/toViewImage.do?taskId='+row[0].id;
	}else{
...
function del2(){
	var row = $("#dg").datagrid('getSelections');
	if(row.length!=0) {
		window.location='${pageContext.request.contextPath}/activiti/activiti/toUpdate.do?id='+row[0].id;
	}else{
		$.messager.alert('提示','请选择一条');
	}
}
...
<body>
   ...
   <table id="dg"  class="easyui-datagrid"  fit="true" 
            url="${pageContext.request.contextPath}/activiti/activiti/list.do"
