	...
	if(row.length!=0) {
		 $.messager.confirm("确认", "确认删除吗？", function (r) {
		        if (r) {
		        	...
		        	$.ajax({   
					     url:'${pageContext.request.contextPath}/sys/scheduleJob/delete.do',   
					     type:'post',   
...
function upd(){
	var row =  $("#dg").datagrid('getSelections');
	if(row.length==1){
		window.location="${pageContext.request.contextPath}/sys/scheduleJob/toUpdate.do?name="+row[0].name+"&group="+row[0].group+"&cronExpression="+row[0].cronExpression+"&className="+row[0].className;
	}else{
		$.messager.alert('提示','请选择一条');
	}
}
	...
	if(row.length!=0) {
		 $.messager.confirm("确认", "确认操作吗？", function (r) {
		        if (r) {
		        	...
		        	$.ajax({   
					     url:'${pageContext.request.contextPath}/sys/scheduleJob/startNow.do',   
					     type:'post',   
					     data:"name="+myArray+"&group="+myArray2,
					     success:function(data){   
					        if(data.msg==null){
	...
	if(row.length!=0) {
		 $.messager.confirm("确认", "确认操作吗？", function (r) {
		        if (r) {
		        	...
		        	$.ajax({   
					     url:'${pageContext.request.contextPath}/sys/scheduleJob/resume.do',   
					     type:'post',   
					     data:"name="+myArray+"&group="+myArray2,
					     success:function(data){   
					        if(data.msg==null){
	...
	if(row.length!=0) {
		 $.messager.confirm("确认", "确认操作吗？", function (r) {
		        if (r) {
		        	...
		        	$.ajax({   
					     url:'${pageContext.request.contextPath}/sys/scheduleJob/stop.do',   
					     type:'post',   
					     data:"name="+myArray+"&group="+myArray2,
					     success:function(data){   
					        if(data.msg==null){
...
function cron(){
	window.location="${pageContext.request.contextPath}/sys/scheduleJob/cron.do";
}
...
<body>
   ...
   <table id="dg"  class="easyui-datagrid"  fit="true" 
            url="${pageContext.request.contextPath}/sys/scheduleJob/list.do"
            toolbar="#toolbar" 
    ...
    <div id="toolbar" >
			...
			<a href="${pageContext.request.contextPath}/sys/scheduleJob/toAdd.do" class="easyui-linkbutton" iconCls="icon-add" plain="true" >新增</a>
