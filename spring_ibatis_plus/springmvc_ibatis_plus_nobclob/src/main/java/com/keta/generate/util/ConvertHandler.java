...
public class ConvertHandler
{
     ...
     * @throws Exception
     */
    public static void tableHandle(Table table)
        throws Exception
    {
    ...
    /**
     * 进行转换时，主键ID为Long 类型，pojo均继承IdEntity，所以pojo里不需要再次声明ID变量。 需要从columns中清除该列，避免pojo重复生成
     */
    private static void cleanColumn(Table table)
    {
        if (table.getPk() != null && table.getColumns() != null)
        {
            for (Column col : table.getColumns())
            {
                col.setComment(StringUtils.trimToEmpty(col.getComment()));
