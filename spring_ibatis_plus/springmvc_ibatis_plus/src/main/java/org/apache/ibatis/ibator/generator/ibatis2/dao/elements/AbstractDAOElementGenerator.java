...
public abstract class AbstractDAOElementGenerator extends AbstractGenerator
{
    ...
    public abstract void addJunitElements(JunitTestClass junitClass);
    
    public DAOMethodNameCalculator getDAOMethodNameCalculator()
    {
        if (dAOMethodNameCalculator == null)
        {
            String type = ibatorContext.getDaoGeneratorConfiguration().getProperty(PropertyRegistry.DAO_METHOD_NAME_CALCULATOR);
            ...
            try
            {
                dAOMethodNameCalculator = (DAOMethodNameCalculator)IbatorObjectFactory.createInternalObject(type);
            }
            catch (Exception e)
            {
    ...
    public JavaVisibility getExampleMethodVisibility()
    {
        ...
        {
            String type = ibatorContext.getDaoGeneratorConfiguration().getProperty(PropertyRegistry.DAO_EXAMPLE_METHOD_VISIBILITY);
            if (StringUtility.stringHasValue(type))
            {
