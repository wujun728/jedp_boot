...
public class DefaultDAOMethodNameCalculator implements DAOMethodNameCalculator
{
    ...
    /**
     * 1. if this will be the only updateByPrimaryKey, then the result should be updateByPrimaryKey. 2. If the other method is enabled, but there are seperate base and blob classes, then the method
     * name should be updateByPrimaryKey 3. Else the method name should be updateByPrimaryKeyWithoutBLOBs
     */
    @Override
    ...
    /**
     * 1. if this will be the only updateByPrimaryKey, then the result should be updateByPrimaryKey. 2. If the other method is enabled, but there are seperate base and blob classes, then the method
     * name should be updateByPrimaryKey 3. Else the method name should be updateByPrimaryKeyWithBLOBs
     */
    @Override
    ...
    @Override
    public String getSelectAllWithoutBLOBsMethodName(IntrospectedTable introspectedTable)
    {
        IbatorRules rules = introspectedTable.getRules();
        if (!rules.generateSelectByExampleWithBLOBs())
        {
            return "selectAll"; //$NON-NLS-1$
        }
        else
        {
            return "selectAllWithoutBLOBs"; //$NON-NLS-1$
        }
    }
    
    @Override
    public String getSelectAllWithBLOBsMethodName(IntrospectedTable introspectedTable)
    {
        IbatorRules rules = introspectedTable.getRules();
        if (rules.generateSelectByExampleWithBLOBs())
        {
        ...
        {
            return "selectAllWithBLOBs"; //$NON-NLS-1$
        }
    }
