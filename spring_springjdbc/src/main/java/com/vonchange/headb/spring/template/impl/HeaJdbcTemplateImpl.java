...
/**
 * <b>This is the central class in the JDBC core package.</b> It simplifies the
 * use of JDBC and helps to avoid common errors. It executes core JDBC workflow,
 * leaving application code to provide SQL and extract results. This class
 * executes SQL queries or updates, initiating iteration over ResultSets and
 ...
 * clearly defined contract. The {@link PreparedStatementCreator} callback
 ...
 * DataSource reference, or get prepared in an application context and given to
 * services as bean reference. Note: The DataSource should always be configured
 * as a bean in the application context, in the first case given to the service
 ...
 * {@link org.springframework.jdbc.support.SQLExceptionTranslator} interface,
 ...
 */
public class HeaJdbcTemplateImpl extends JdbcAccessor implements IHeaJdbcTemplate {
	 ...
	 * bypassed. All other results processing will be take place unless the
	 * variable {@code skipResultsProcessing} is set to {@code true}.
	 */
	private boolean skipUndeclaredResults = false;

	/**
	 * If this variable is set to true then execution of a CallableStatement
	 * will return the results in a Map that uses case insensitive names for the
	 * parameters if Commons Collections is available on the classpath.
	 */
	 ...
	 */
	public HeaJdbcTemplateImpl(DataSource dataSource) {
	 ...
	 * from.
	 * <p>
	 * Note: Depending on the "lazyInit" flag, initialization of the exception
	 * translator will be triggered.
	 * 
	 * @param dataSource
	 *            the JDBC DataSource to obtain connections from
	 * @param lazyInit
	 ...
	 */
	public HeaJdbcTemplateImpl(DataSource dataSource, boolean lazyInit) {
		setDataSource(dataSource);
	 ...
	 * application.
	 * <p>
	 * Default is -1, indicating to use the JDBC driver's default (i.e. to not
	 * pass a specific fetch size setting on the driver).
	 * 
	 ...
	 * <p>
	 * Default is -1, indicating to use the JDBC driver's default (i.e. to not
	 * pass a specific max rows setting on the driver).
	 * 
	 ...
	 * <p>
	 * Default is -1, indicating to use the JDBC driver's default (i.e. to not
	 * pass a specific query timeout setting on the driver).
	 * <p>
	...
	public <T> T execute(ConnectionCallback<T> action) throws DataAccessException {
		...
		try {
			...
			if (this.nativeJdbcExtractor != null) {
				// Extract native JDBC Connection, castable to OracleConnection
				// or the like.
	...
	/**
	 * Create a close-suppressing proxy for the given JDBC Connection. Called by
	 * the {@code execute} method.
	 * <p>
	 * The proxy also prepares returned JDBC Statements, applying statement
	 * settings such as fetch size, max rows, and query timeout.
	 * 
	 * @param con
	 *            the JDBC Connection to create a proxy for
	 ...
	 *            Parameter list of declared resultSet parameters for the stored
	 *            procedure
	 ...
	 * <p>
	 * If "isResultsMapCaseInsensitive" has been set to true, a linked
	 * case-insensitive Map will be created.
	 * 
	 ...
	 * <p>
	 * By default, we'll create an {@link ArgumentPreparedStatementSetter}. This
	 * method allows for the creation to be overridden by subclasses.
	 * 
	 * @param args
	 *            object array with arguments
	 ...
	 *            object array with arguments
	 * @param argTypes
	 *            int array of SQLTypes for the associated arguments
	 * @return the new PreparedStatementSetter to use
	 */
	 ...
	 * 
	 * @param warning
	 *            the warnings object from the current statement. May be
	 *            {@code null}, in which case this method does nothing.
	 * @throws SQLWarningException
		...
		return execute(psc, new PreparedStatementCallback<T>() {
			...
			public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
				...
				try {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("生成的主键为: " + generatedKeys);
					}
	...
	protected <T> T queryProc(final PreparedStatementCreator psc, final PreparedStatementSetter pss, final ResultSetExtractor<T> rse) throws DataAccessException {
		logger.debug("Executing prepared SQL update");
		return execute(psc, new PreparedStatementCallback<T>() {
			public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
				T result = null;
				try {
					...
					if(flag){
						...
						result = rse.extractData(resultSet);
					}
					return result;
				} finally {
					if (pss instanceof ParameterDisposer) {
	...
	private <T> T queryProc(String sql, PreparedStatementSetter pss, ResultSetExtractor<T> rse)  {
		return insert(new InsertPreparedStatementCreator(sql), pss, rse);
	}

	@Override
	public <T> T queryProc(String sql, ResultSetExtractor<T> rse, Object... params) throws SpringJdbcDataAccessException   {
		if (sql.toUpperCase().indexOf("CALL") == -1) {
			throw new SpringJdbcDataAccessException("Not a callable statement");
		}
		return queryProc(sql, newArgPreparedStatementSetter(params), rse);
	}
