...
public class RecordWithBLOBsGenerator extends BaseModelClassGenerator
{
    ...
    public List<CompilationUnit> getCompilationUnits()
    {
        ...
        progressCallback.startTask(Messages.getString("Progress.9", table.toString())); //$NON-NLS-1$
        ...
        for (IntrospectedColumn introspectedColumn : introspectedTable.getBLOBColumns())
        {
            ...
            if (plugins.modelFieldGenerated(field, topLevelClass, introspectedColumn, introspectedTable, IbatorPlugin.ModelClassType.RECORD_WITH_BLOBS))
            {
            ...
            if (plugins.modelGetterMethodGenerated(method, topLevelClass, introspectedColumn, introspectedTable, IbatorPlugin.ModelClassType.RECORD_WITH_BLOBS))
            {
                topLevelClass.addMethod(method);
            }
            
            method = getJavaBeansSetter(introspectedColumn);
            if (plugins.modelSetterMethodGenerated(method, topLevelClass, introspectedColumn, introspectedTable, IbatorPlugin.ModelClassType.RECORD_WITH_BLOBS))
            {
                topLevelClass.addMethod(method);
            }
        }
