...
/**
 * This class encapsulates all the code generation rules for a table using the conditional model. In this model we do not generate primary key or record with BLOBs classes if the class would only hold
 * one field.
 * 
 * @author Jeff Butler
 * 
...
public class ConditionalModelRules extends IbatorRules
{
    ...
    /**
     * Generate a base record if there are any base columns, or if there is only one primary key coulmn (in which case we will not generate a primary key class), or if there is only one BLOB column
     * (in which case we will not generate a record with BLOBs class).
     * 
