...
import com.xiongyw.plugin.Constant;
import com.xiongyw.plugin.PagePlugin;
import com.xiongyw.plugin.PageView;
import com.xiongyw.util.Common;
import com.xiongyw.util.EhcacheUtils;
import com.xiongyw.util.StringUtil;
	...
	public static final String joinSql(Connection connection, MappedStatement mappedStatement,
		...
		if (Constant.FINDBYWHERE.equals(sqlId)) {
			...
			for (Entry<String, Object> entry : formMap.entrySet()) {
				if (!Constant.SYS_TABLE.equals(entry.getKey()) && null != entry.getValue()
			...
			sql = "insert into " + table.toString() + " (" + StringUtil.trimComma(fieldString)
					+ ")  values (" + StringUtil.trimComma(fieldValues) + ")";
			...
			sql = "update " + table.toString() + " set " + StringUtil.trimComma(fieldString)
					+ " " + where;
			...
			if (null != formMaps && formMaps.size() > 0) {
				table = Common.toHashMap(formMaps.get(0)).get(Constant.SYS_TABLE);
				mapfield = (Map<String, Object>) EhcacheUtils.get(table);
			...
			for (int i = 0; i < formMaps.size(); i++) {
				...
				fvs += "(" + StringUtil.trimComma(fieldValues) + "),";
				fs += " insert into " + table.toString() + " ("
						+ StringUtil.trimComma(fieldString) + ")  values ("
						+ StringUtil.trimComma(fieldValues) + ");";
				fieldValues = "";
				fieldString = "";
			}
			String v = StringUtil.trimComma(fvs);
			sql = "insert into " + table.toString() + " (" + StringUtil.trimComma(fd)
					+ ")  values " + StringUtil.trimComma(fvs) + "";
			String[] vs = v.split("\\),");
			boolean b = false;
